...
export default function Login() {
  ...
  async function loginSubmit(data) {
    try {
      ...
      if (respo.data.success === true) {
        ...
        localStorage.setItem("UserToken", respo.data.data.token);
        settokenUser(respo.data.data.token);
        localStorage.setItem("UserData", JSON.stringify(respo.data.data.user));
        setuserData(respo.data.data.user);
        navigate("/home");
      }
    } catch (err) {
      setapiError(err.response?.data?.errors || "Something went wrong");
      setloading(false);
    }
  }
  ...
  return (
    <div className="grid lg:grid-cols-2 min-h-screen w-full font-sans antialiased h-100 overflow-hidden">
      <Toaster position="top-right" reverseOrder={false} />
      <div
        className="signup-hero text-white flex flex-col justify-between p-10 relative overflow-hidden"
        style={{
      ...
      >
        ...
        <div className="content space-y-8 relative z-10 py-2">
          <div className="title">
      ...
      <div className="login-form bg-slate-50 py-6 px-6 flex justify-center items-center">
        ...
        >
          ...
          <button
            ...
            type="submit"
            className="cursor-pointer disabled:cursor-not-allowed  disabled:bg-slate-500 w-full py-4 bg-slate-800 text-white rounded-2xl font-black text-lg hover:bg-slate-900 shadow-xl shadow-slate-200 transition-all flex items-center justify-center gap-3"
          >
            <span>Sign in</span>
